...
///
/// Design: the watch sends each completed session as an application-context
/// dictionary (keyed by `sessionKey`). The phone decodes it on arrival, appends
/// to the store, and publishes a `lastReceivedAt` so views can react. If the
/// same session id arrives twice (WC can redeliver), it is deduped.
///
...
final class WatchSyncManager: NSObject, ObservableObject {
    ...
    /// JSON-encoded `HangSession` arrives under this key in the application
    /// context. Shared string with the watch sender (defined in
    /// `HangConnectivity` so the watch can reference it too).
    ...
    private func ingest(payload: [String: Any], into store: HangSessionStore) {
